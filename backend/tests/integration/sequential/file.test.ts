...
test('freezing existing file', async () => {
  ...
  await expect(axios.post(`${backendPrivateUrl}files/`, {uuid: volatileFile.uuid, volatile: false, pid: '1234'})).resolves.toMatchObject({status: 200})
  const dbRow = await fileRepo.findOneOrFail(volatileFile.uuid)
  expect(dbRow.volatile).toEqual(false)
  expect(dbRow.pid).toEqual('1234')
})
