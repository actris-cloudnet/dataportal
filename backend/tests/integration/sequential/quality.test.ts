...
describe("PUT /quality/:uuid", () => {
  ...
  it("on new report responds with 201 and creates report", async () => {
    ...
    await expect(axios.get(`${fileUrl}acf78456-11b1-41a6-b2de-aa7590a75675`)).resolves.toMatchObject({
      status: 200,
      data: { qualityScore: validPayload.overallScore },
    });
  ...
  it("on existing model file report responds with 200 and updates report", async () => {
    await expect(axios.put(`${privateUrl}b5d1d5af-3667-41bc-b952-e684f627d91c`, validPayload)).resolves.toMatchObject({
      status: 201,
    });
    const tmpPayload = { ...validPayload, overallScore: 0.8 };
    await expect(axios.put(`${privateUrl}b5d1d5af-3667-41bc-b952-e684f627d91c`, tmpPayload)).resolves.toMatchObject({
      status: 200,
    });
    await expect(axios.get(`${fileUrl}b5d1d5af-3667-41bc-b952-e684f627d91c`)).resolves.toMatchObject({
      status: 200,
      data: { qualityScore: tmpPayload.overallScore },
    });
