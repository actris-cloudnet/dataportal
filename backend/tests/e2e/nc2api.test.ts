...
const inputJson = {
  ...
  s3key: "something/20190723_bucharest_classification.nc",
...
describe("after PUTting metadata to API", () => {
  beforeAll(async () => {
    await axios.put(`${storageServiceUrl}cloudnet-product-volatile/${inputJson.s3key}`, fs.createReadStream(filepath));
    return axios.put(`${backendPrivateUrl}files/${inputJson.filename}`, inputJson);
  });
  ...
  describe("after PUTting more metadata to API", () => {
    ...
    beforeAll(async () => {
      ...
      await axios.put(`${storageServiceUrl}cloudnet-product-volatile/${filename}`, fs.createReadStream(filepath));
      await axios.put(`${backendPrivateUrl}files/${filename}`, inputJson2);
