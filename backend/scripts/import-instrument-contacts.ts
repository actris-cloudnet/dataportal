...
import { normalizeOrcid } from "../src/lib/cite";
  ...
  for (const instrument of instruments) {
    try {
      ...
      for (const pi of pis) {
        ...
        if (pi.orcid_id) {
          const normalizedOrcid = normalizeOrcid(pi.orcid_id);
          ...
          person = await personRepo.findOneBy({ firstName: pi.first_name, lastName: pi.last_name });
          if (!person) {
            person = personRepo.create({ firstName: pi.first_name, lastName: pi.last_name });
            person = await personRepo.save(person);
          }
        }
