...
async function createServer(): Promise<void> {
  ...
  if (env.ORCID_CLIENT_ID && env.ORCID_CLIENT_SECRET) {
    ...
    app.get("/api/auth/orcid", authenticator.nextCookie, passport.authenticate("orcid", { session: false }));
    app.get(
      "/api/auth/orcid/callback",
      passport.authenticate("orcid", {
        session: false,
        failureRedirect: `${env.DP_FRONTEND_URL}/login?fail=orcid`,
      }),
  ...
  app.get("/api/sites/:siteId", middleware.optionalAuth, siteRoutes.site);
