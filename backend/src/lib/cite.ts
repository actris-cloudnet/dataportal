...
export class CitationService {
  ...
  async getCollectionCitation(collection: Collection): Promise<Citation> {
    const [instrumentPis, usesModelData, nfPis, productNames, sites, dateRange, collectionSiteData] = await Promise.all(
      [
        ...
        this.queryCollectionSiteData(collection),
      ],
    );
    if (usesModelData || productNames.includes("Model")) {
      instrumentPis.push(MODEL_AUTHOR);
    }
    const siteAuthors = await this.fetchSitePis(collectionSiteData);
  ...
  for (const item of data) {
    ...
    for (const date of item.dates) {
      for (const contact of itemContacts) {
        ...
        if (inRange) {
          const dedup = contact.person.orcid ?? `${contact.person.firstname}|${contact.person.surname}`;
          if (!seen.has(dedup)) {
            seen.add(dedup);
            result.push({
              firstName: contact.person.firstname,
              lastName: contact.person.surname,
    ...
    await Promise.all(
      data.map((item) =>
        fetchNfPi(item.actrisId).then((contacts) => {
          const output = new Set<NfContact>();
          for (const date of item.dates) {
            contacts
              .filter(
                (contact) =>
