...
export class AddQualityScoreToSearchFile1630930366314 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE search_file SET "qualityScore" = regular_file."qualityScore" FROM regular_file WHERE search_file.uuid = regular_file.uuid`,
    );
    await queryRunner.query(
      `UPDATE search_file SET "qualityScore" = model_file."qualityScore" FROM model_file WHERE search_file.uuid = model_file.uuid`,
    );
  }
