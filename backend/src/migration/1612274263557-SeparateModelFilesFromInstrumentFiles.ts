...
export class SeparateModelFilesFromInstrumentFiles1612274263557 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "model_file" ("uuid" uuid NOT NULL, "s3key" character varying NOT NULL, "version" character varying NOT NULL, "pid" character varying NOT NULL DEFAULT '', "volatile" boolean NOT NULL DEFAULT true, "legacy" boolean NOT NULL DEFAULT false, "measurementDate" date NOT NULL, "history" character varying NOT NULL DEFAULT '', "checksum" character varying NOT NULL, "size" integer NOT NULL, "format" character varying NOT NULL, "createdAt" TIMESTAMP NOT NULL, "updatedAt" TIMESTAMP NOT NULL, "siteId" character varying, "productId" character varying, "modelId" character varying, CONSTRAINT "UQ_c12dee99fecddb342ac1b1c865d" UNIQUE ("checksum"), CONSTRAINT "PK_fc253af1bf074f42da1c667ac6b" PRIMARY KEY ("uuid"))`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "model_visualization" ("s3key" character varying NOT NULL, "sourceFileUuid" uuid, "productVariableId" character varying, CONSTRAINT "PK_f823f103aacdc71693293a1a4b8" PRIMARY KEY ("s3key"))`,
    );
    await queryRunner.query(
    ...
    // COLLECTION
    await queryRunner.query(
      `CREATE TABLE "collection_model_files_model_file" ("collectionUuid" uuid NOT NULL, "modelFileUuid" uuid NOT NULL, CONSTRAINT "PK_91daea83f0e0b82fe8e4581ab6d" PRIMARY KEY ("collectionUuid", "modelFileUuid"))`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_694ee45a95d4b031ff9776cbe8" ON "collection_model_files_model_file" ("collectionUuid") `,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_27fd775d0627f97133310c77d0" ON "collection_model_files_model_file" ("modelFileUuid") `,
    );
    await queryRunner.query(
    ...
    // MOVE DATA (CUSTOM)
    await queryRunner.query(
      `INSERT INTO model_file SELECT uuid, s3key, version, pid, volatile, legacy, "measurementDate", history, checksum, size, format, "createdAt", "updatedAt", "siteId", "productId", "modelId" FROM regular_file WHERE "productId" = 'model'`,
    );

    await queryRunner.query(
      `INSERT INTO model_visualization SELECT * FROM visualization WHERE "sourceFileUuid" IN (SELECT uuid FROM model_file)`,
    );
    await queryRunner.query(`DELETE FROM visualization WHERE "sourceFileUuid" IN (SELECT uuid FROM model_file)`);
    ...
    await queryRunner.query(
      `INSERT INTO collection_model_files_model_file SELECT * FROM collection_files_regular_file WHERE "fileUuid" IN (SELECT uuid FROM model_file)`,
    );
    await queryRunner.query(
      `DELETE FROM collection_files_regular_file WHERE "fileUuid" IN (SELECT uuid FROM model_file)`,
    );
  }
