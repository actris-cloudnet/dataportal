...
We provide an HTTP REST API for programmatical access to the metadata at the cloudnet data portal. The API responds with JSON and can be accessed via the base URL `https://altocumulus.fmi.fi/api/`. Metadata can be queried via the following routes.
