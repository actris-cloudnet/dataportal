...
const getShadowIcon = () => require("../assets/markers/marker-shadow.png");
...
export default class Map extends Vue {
  ...
  // markers
  marker = (site: Site, selected: boolean) =>
    L.Icon.extend({
      options: {
  ...
  initBoundingBoxTool() {
    ...
    map.on(L.Draw.Event.CREATED, (e) => {
      ...
      const clickedSites = keys.filter((key) =>
        L.latLngBounds(e.layer.getBounds()).contains(this.allMarkers[key].getLatLng())
      );
  ...
  getMapCenter() {
    ...
    if (this.showLegend) center = [34.0, 14.0];
    return center;
  }

  setMapBounds() {
    const southWest = L.latLng(-90.0, -330.0),
      northEast = L.latLng(90.0, 330.0),
