...
-->
      <h4>Citation</h4>
      <div class="citationtext">{{ datasetCreator }}: cloud profiling product{{ products.length > 1 ? 's' : ''}} {{ products.map(product => product.humanReadableName).join(', ')}}; {{ startDate + (endDate ? ` - ${endDate}` : '') }}; {{ sites.map(site => site.humanReadableName).join(', ')}}. Generated by the cloud profiling unit of the ACTRIS Data Centre, <a :href="pid">{{ pid }}</a>, {{ year }}.</div>
    </div>
  </section>
</template>
