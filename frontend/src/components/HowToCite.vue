...
-->
      ...
      <div class="citationtext">
        ...
        from {{ sites.map(site => site.humanReadableName).join(', ')}}.
        Generated by the cloud profiling unit of the ACTRIS Data Centre, <a :href="pid">{{ pid }}</a>, {{ year }}.</div>
    </div>
  </section>
</template>
...
export default class HowToCite extends Vue {
  ...
  get productsText() {
    ...
    return `cloud profiling product${products.length > 1 ? 's' : ''}: ${products.map(prod => prod.humanReadableName).join(', ')}; `
  }

  get measurementsText() {
    const measurements = this.products.filter(prod => parseInt(prod.level) == 1 && prod.id != 'categorize' && prod.id != 'model')
  ...
  mounted() {
    ...
    this.citations = instrSites.filter(getCitations).map(getCitations)
      .concat(this.models.filter(getCitations).map(getCitations)).flat()
  }
