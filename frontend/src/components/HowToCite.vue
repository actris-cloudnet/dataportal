  ...
  <section id="howtocite">
    ...
    <div class="infobox infocolor">
      ...
      <div class="citationtext">
        ...
        using the following link: <a :href="pid">{{ pid }}</a
        >.
      </div>
      <h4>Acknowledgements</h4>
      <div class="citationtext">
        We acknowledge ACTRIS for providing the {{ datasetCreator }} dataset in this study, which was produced by the
        Finnish Meteorological Institute, and is available for download from
...
-->
      <h4>Citation</h4>
      <div class="citationtext">
        {{ datasetCreator }}: {{ allProductsText }} {{ startDate + (endDate ? ` to ${endDate}` : "") }}; from
        {{ sites.map((site) => site.humanReadableName).join(", ") }}. Generated by the cloud profiling unit of the
        ACTRIS Data Centre, <a :href="pid">{{ pid }}</a
        >, {{ year }}.
      </div>
    </div>
  </section>
...
export default class HowToCite extends Vue {
  ...
  get productsText() {
    const products = this.products.filter((prod) => parseInt(prod.level) > 1 || prod.id == "categorize");
  ...
  get measurementsText() {
    const measurements = this.products.filter(
      (prod) => parseInt(prod.level) == 1 && prod.id != "categorize" && prod.id != "model"
    );
    if (!measurements.length) return "";
    return `cloud profiling measurements: ${measurements.map((prod) => prod.humanReadableName).join(", ")}; `;
  }

  get modelsText() {
    if (!this.models.length) return "";
    return `${this.models.map((model) => model.id).join(", ")} model data; `;
  }

  get allProductsText() {
    return this.capitalizeFirstLetter(`${this.productsText}${this.measurementsText}${this.modelsText}`);
  }
  ...
  mounted() {
    const instrSites = this.sites.filter((site: Site) => this.nonModelSiteIds.includes(site.id));
    const getCitations = (obj: Site | Model) => obj.citations as Citation[];
    this.citations = instrSites
      .filter(getCitations)
      .map(getCitations)
      .concat(this.models.filter(getCitations).map(getCitations))
      .flat();
  }
}
</script>
