...
-->
      <h4>Citation</h4>
      <div class="citationtext">{{ datasetCreator }}: cloud profiling product{{ products.length > 1 ? 's' : ''}} {{ products.join(', ')}}; {{ startDate + (endDate ? ` - ${endDate}` : '') }}; {{ siteIds.join(', ')}}. Generated by the cloud profiling unit of the ACTRIS Data Centre, <a :href="pid">{{ pid }}</a>, {{ year }}.</div>
    </div>
  </section>
</template>
...
export default class HowToCite extends Vue {
  ...
  created() {
    return axios
      .get(`${this.apiUrl}citation`, { params: { site: this.siteIds}})
      .then(response => {
        this.acknowledgements = response.data.map((citation: Citation)=> citation.acknowledgements)
      })
  }
