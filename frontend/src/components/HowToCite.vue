...
function uniqueCitations() {
  const uniqueIds = Array.from(new Set(citations.map((cit) => cit.id)));
  return uniqueIds.map((id) => citations.find((cit) => cit.id == id));
}

function productsText() {
  const products = props.products.filter((prod) => parseInt(prod.level) > 1 || prod.id == "categorize");
...
function allProductsText() {
  return capitalizeFirstLetter(`${productsText()}${measurementsText()}${modelsText()}`);
}

onMounted(() => {
  const instrSites = props.sites.filter((site: Site) => props.nonModelSiteIds.includes(site.id));
  const getCitations = (obj: Site | Model) => obj.citations as Citation[];
  citations = instrSites
    .filter(getCitations)
    .map(getCitations)
