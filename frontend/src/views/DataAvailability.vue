...
const dataStatusParsers = ref<Record<string, DataStatusParser>>({});
...
async function initDataStatusParser(site: Site) {
  ...
  dataStatusParsers.value[site.id] = await new DataStatusParser(payload).engage();
...
async function refreshViz() {
  ...
  const dateFromYear = yearRange.value === "11" ? 1971 : yearNow - parseInt(yearRange.value);
...
onMounted(async () => {
  ...
  sites.value = res.data.filter((site: Site) => site.type.includes("cloudnet" as SiteType));
  await refreshViz();
});

const evenSites = computed(() => sites.value.filter((site, index) => !(index % 2)));
